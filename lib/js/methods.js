...
const heroCreator = {
    heroCreator: populateGrid = function(gridDiv, hero_array){
        for(const hero of hero_array){
            const charSheet = createCharSheet(hero); 
    ...
    heroCreator: createCharSheet = function(heroObj){
        ...
        image.src = heroObj.img;
        newDiv.appendChild(image);
        newDiv.appendChild(this.createInfoDiv(heroObj));
        return newDiv;
    },
    heroCreator: createInfoDiv = function(heroObj){
        let textHeroInfo = document.createElement("div");
        newDiv.classList.add("hero_text_container");
        let arr = Object.keys(heroObj);
        for(string of arr) {
            if(Array.isArray(string)){
                string = getAbilityString(string);
            }
            let p = document.createElement("p");
            p.innerHTML = string + ": " + string.value;
            textHeroInfo.appendChild(p); 
        }
    },
    heroCreator: getAbilityString = function(abilitiesArr){
        let str = "";
        for(ability of abilitiesArr) {
            str += (ability + ", ");
        }
        return str;
    }
...
let parentDiv = document.querySelector("#gridDiv");
populateGrid(parentDiv, heroes);
